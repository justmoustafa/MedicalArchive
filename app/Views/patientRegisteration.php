...
<head>
    ...
    .registration form input, .custom-file, .custom-date {
      display: block;
      width: 50%;
      margin: 20px auto;
      padding: 8px 7px;
    ...
    @media (max-width: 767.98px) {
      .registration form input, .registration form button[type="submit"], .custom-file, .custom-date  {
        width: 95%;
      } 
    }
    </style>
...
<body>
 ...
 <?php 
        ...
        <form action="patientRegisteration" method='post' enctype="multipart/form-data">
<?php
                    if(isset($validation)):
                        if($validation->hasError('patientId')):?>
							<p class="alert alert-danger">
								<?php echo $validation->getError('patientId');?>
							</p>
                <?php
                        endif;
					endif;?> 

            <input type="text" name='patientId' placeholder="national id" value=<?= set_value('patientId')?>>
<?php
